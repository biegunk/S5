...
from s5dev.models.utils import Identity, StochasticDepth
...
class Mlp(nn.Module):
    ...
    def setup(self):
        ...
        out_kernel_init = flax_normal(
            stddev=self.initializer_range / math.sqrt(2 * self.n_layer)
        )
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        if self.prenorm:
            ...
            dropped = self.drop_path1(
                self.dropout1(deterministic=not training)(hidden_states), training
            )
            ...
            if not isinstance(self.mlp, Identity):
                dropped = self.drop_path2(
                    self.dropout2(deterministic=not training)(hidden_states), training
                )
                residual = (dropped + residual) if residual is not None else dropped
            ...
            hidden_states = self.norm1(
                self.drop_path1(
                    self.dropout1(deterministic=not training)(mixer_out), training
                )
                + hidden_states
            )
            ...
            if not isinstance(self.mlp, Identity):
                ...
                hidden_states = self.norm2(
                    self.drop_path2(
                        self.dropout2(deterministic=not training)(mlp_out), training
                    )
                    + hidden_states
                )
    ...
    def step(
        ...
        if self.prenorm:
            ...
            new_mixer_state, hidden_state = self.mixer.step(
                mixer_state, hidden_state, **mixer_kwargs
            )

            if mixer_subset is not None:
                residual = residual[
                    :, mixer_subset
                ]  # Take a subset before applying the query projectin.
            ...
            assert residual is None
            new_mixer_state, mixer_out = self.mixer.step(
                mixer_state, hidden_state, **mixer_kwarg
            )
...
def create_mixer_cls(
    ...
    if (attn_layer_idx is not None) and (layer_idx in attn_layer_idx):
        ...
        if layer.lower() == "hyena":
            ...
            mixer_cls = S5Operator(
                d_model, n_layer, l_max, filter_cls="identity", **layer_kwargs
            )

        elif layer.lower() == "rotrnn":
            mixer_cls = S5Operator(
                d_model, n_layer, l_max, filter_cls="rotrnn", **layer_kwargs
            )
...
def create_mlp_cls(d_model, d_inner=None):
    ...
    mlp_cls = partial(
        Mlp, hidden_features=inner_dim, activation=partial(nn.gelu, approximate=True)
    )
...
class LMBackbone(nn.Module):
    ...
    def step(self, layer_states, hidden_state):
        ...
        for layer, layer_state in zip(self.layers, layer_states):
            new_layer_state, hidden_state, residual = layer.step(
                layer_state, hidden_state, residual
            )
